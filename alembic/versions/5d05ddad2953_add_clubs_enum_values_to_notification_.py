...
def upgrade() -> None:
    ...
    op.execute(
        "ALTER TYPE notificationeventtype ADD VALUE IF NOT EXISTS 'CLUB_JOIN_REQUEST'"
    )
    op.execute(
        "ALTER TYPE notificationeventtype ADD VALUE IF NOT EXISTS 'CLUB_JOIN_ACCEPTED'"
    )
    op.execute(
        "ALTER TYPE notificationeventtype ADD VALUE IF NOT EXISTS 'CLUB_JOIN_REJECTED'"
    )
